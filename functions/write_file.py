...
def write_file(working_directory, file_path, content):
    ...
    if not abs_file_path.startswith(abs_working_dir):
        return f'Error: Cannot write to "{file_path}" it is not a working directory'
    ...
    if not os.path.isdir(parent_dir):
        try:
            ...
            return f'Je ne peux pas créer les répertoires "{parent_dir}": {e}'
        
        
    try:
        with open(abs_file_path, "w") as f:
            f.write(content)
        return f'Success: Wrote to file "{file_path}"({len(content)} characters)'
    except Exception as e:
